...
#SBATCH -t 10:00:00
#SBATCH -p general # micro for nodes<=16, general for nodes>16
...
taskspernode=2
portfolio=kcl
timeout="$((10*3600 - 60))" # 10 hours - epsilon
benchmarkfile="benchmark.txt"
nbenchmarks=$(cat ${benchmarkfile}.0|wc -l)
disturbconfig="p3"

cmd="
build/mallob -q -c=2 -ajpc=$nparjobs -sleep=1000 -ljpc=8 -SJ=$nbenchmarks -T=$timeout -log=$logdir -v=4 \
-job-template=instances/job-template-isc22.json -job-desc-template=$benchmarkfile \
-client-template=templates/client-template-disturbances-${disturbconfig}.json -sro=${logdir}/processed-jobs.out -sjd=1 -jwl=300 \
...
`#sharingvolume` -s=0.5 -cbbs=4500 -cblm=1 -cblp=250000 -cusv=1 \
`#randomseed` -seed=0
"
